...
- [ ] replicate some big projects using only slap to confirm it works
  ...
  - open: **the parse ceiling moved rather than disappeared.** Around 17k bytes of source the record built by `into` exceeds LOCAL_MAX; past that the frame arena fills. Same shape of fix as `case` where the copy is avoidable. `tests/run_feed.py` brackets both ends.
  - open: **parse.slap's combinators recurse once per character, and `parse-spaces-core` SEGFAULTS at about 3000 consecutive spaces.** Pre-existing and independent of the above (3000 is well under the old cap; it was simply unreachable through `then`). `parse-while`/`parse-int` fill the frame arena at about 5000 with a clean error instead. Two things to do: (a) `EVAL_DEPTH_MAX` is 10000, too high to catch C-stack exhaustion first — either lower it or, better, measure actual C-stack usage against a base recorded in `main` so the guard is shape-independent and a bare SIGSEGV becomes an Elm-style message; (b) make `parse-spaces-core`, `parse-while-acc`, `parse-int-acc`, `parse-float-int/frac` and `parse-until` iterative. (b) is a redesign, not an edit: the "remaining input on the stack" convention wants `swap`/`dip` to step over the input, while an index scan wants the string `let`-bound, which is an O(n) frame copy per call. Long documents are fine today; a single long token run is not.
