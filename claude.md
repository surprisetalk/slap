...
13. `python3 tests/run_feed.py` — renders `examples/feed.slap` over both checked-in fixtures (RSS 2.0 and Atom), checks the failure paths report instead of printing an empty digest, and pins the parse ceiling from both sides at the exact measured boundary (116 items renders, 117 does not)
...
`LOCAL_MAX` (16384) bounds the C-stack scratch buffers that primitives use for a single value: `get`/`pop`/`peek`/`nth` element copies, `push`/`insert` operands, `cat` results, `each`/`fold` inputs, `at`/`edit` field copies, `dip`, and `case`'s *predicate-mode* scrutinee. All of them check it and die with a `value too large (N slots, max M)`-style message.

Three that used to be on that list are not any more, and the reason is the same every time — the copy was never needed. Two of the three moved a user-visible ceiling; the third only moved which primitive reports it:
...
- **`into` copies nothing at all.** The caller leaves the new value directly above the record, which is where an appended field's value already belongs: a record is `k v k v … header`, so appending is overwriting the old header slot with the key and pushing a new header over the value that is already there. Replacing an existing key of the same width is one `memmove` into the old field; of a different width it is the same three-reversal rotation `swap` uses, which moves the new value down over the old one and slides the fields after it. `rec_set_field`, which rebuilt the whole record through two `LOCAL_MAX` buffers, is now reached only from `edit`.

`case` and `swap` were the ceiling on every `xd-*`/`jd-*` parse: `parse-exact` and `parse-spaces` carry the *remaining input* through `then`, and `parse-while-core` ends in a `swap` stepping the parsed prefix over it. A feed capped at 16384 bytes of source before those two and parses 16861 bytes now.

**`into` moved the error message, not the ceiling.** `xml.slap` puts a whole document's child list in one element record, so `into` was the next buffer in the chain — but only by 88 slots. Bisected on both binaries with the `tests/run_feed.py` generator: 116 items (16861 bytes) renders and 117 (17008) fails on each. Before, 117 died in `into` at 16472 slots; now it dies in `push` at 16485. What `into` did buy is a record field of any size, which `tests/expect.slap` pins at 20001 and 30001 slots. **It is not uncapped** — past `push` the frame arena fills. `tests/run_feed.py` brackets both ends at that measured boundary.

**parse.slap's combinators are iterative, so a long token run is no longer a separate, lower ceiling than a long document.** They used to recurse once per character, which cost one C frame per character and capped a *token run* far below the document limits above. Measured on the recursive version: `parse-spaces` 3331 spaces, `parse-int` 2933 digits, `parse-float` 2864, `parse-while` 2045 (that one also `let`-bound the accumulator at every level, so it filled the frame arena instead of the C stack).

The shape that replaced it is one `while` loop that scans forward with an index, then one cut:

- `peek` is O(1) for a byte list and leaves the input where it is, so the input never moves during the scan and the loop state is just an index (plus an accumulator where there is one), threaded on the stack because a name cannot be `let` twice in one body.
- Reading past the end returns `'no`, so no length is needed and no sentinel byte is invented — `parse-while`'s caller-supplied predicate still only ever sees a real character, because the `'no` clause stops the loop instead of feeding it a -1.
- `take-n`/`drop-n` move a contiguous run in place, so the cut is one `memmove` rather than a `push` per character.

The limit is now the operand stack rather than the C stack: **2,097,110 spaces** for `parse-spaces` (the whole 2,097,152-slot stack), and **1,048,573** for `parse-while`/`parse-int`/`parse-float`, which halve it because `parse-while-core` and `parse-int-core` `dup` the input. It is also faster, not just deeper: the old shape did a `drop-n` per character, which is O(n) each, so it was quadratic in the run length. 200 runs of 1600 characters, same binary — `parse-spaces` 0.47s → 0.05s, `parse-while` 1.09s → 0.06s, `parse-int` 0.50s → 0.09s, and short tokens (1 to 20 characters) are the same or slightly faster. `tests/expect.slap` pins all five at 100000 characters, which is past every one of the old ceilings and costs about 0.2s.

`parse-until` never recursed — it finds the delimiter with `str-find` and cuts — and `parse-exact` never did either.

**Every write to the operand stack is bounds-checked now.** `spush` always checked, but the bulk-push macro `SPUSH` did not, and neither did `eval_body`'s bare `stack[sp++]=elem` for a literal. `stack` is a global array, so an overrun landed in whatever BSS follows it — `box_walk_depth` in practice, which then reported `box nesting deeper than 512` from a program containing no box. Proved with `-fsanitize=address`: `0 2097150 range ((1 2 3 4 5 6 7 8) drop) apply` wrote 352 bytes past `stack` and still exited 0. `SPUSH` checks per push; the literal path is checked once per body against the body's own slot count, which bounds what its literals can add, so the hot path pays one compare per body rather than one per element. It is not free, but it is small and it is real: interleaved `make bench-uxn` runs give `screen` 18.37/18.44s unguarded against 18.62/18.67s guarded and `drool` 7.33/7.36 against 7.47/7.44 — about 1.3%, in the same direction every pair, so do not describe it as noise.
...
**Deep recursion is guarded by measured C stack use, not by a level count.** `EVAL_DEPTH_MAX` (10000) is a cheap counter and stays as a backstop, but a level costs a different number of C frames depending on shape, so no single count fits: `(inf) 'inf let inf` reaches 10000 levels in 4 MB, a word recursing inside `each` costs 44 KB a level and dies at 169, one recursing inside `edit` costs 512 KB and dies at 14. `main()` records the address of a local in `c_stack_base`, and the `EVAL_DEPTH_MAX` site in `eval_body` compares the address of its own local against it. Past `C_STACK_MAX` (7 MiB) it dies with `C stack exhausted at recursion depth N`. The absolute difference is used, so a stack that grows up is still measured correctly.

7 MiB, not 8, because the suite's deepest program (`euler/27`, three nested recursive words) reaches 6508 KB and the trip point must sit above it, while `die()` needs about 390 KB below it. That leaves the one gap worth knowing about: a `set`-then-`rec_set_field` chain is 1216 KB of C stack and calls no `eval_body`, so no guard sees it, and 1002 KB is not enough room for it. euler/27 leaves only 1662 KB to divide between the two.

**`typecheck_tokens`'s `TypeChecker` is `static` on purpose.** It is about 1.5 MB, and `-flto` inlines the whole function into `main`, so an automatic one made `main`'s frame 1552 KB (`objdump -d --disassemble-symbols=_main slap`) — live for the entire run, on a 8176 KB stack. euler/27 came within 110 KB of a bare SIGSEGV. In BSS the pages are demand-zero and the evaluator gets the whole stack: measured max RSS for chip8 is unchanged at 5.2 MB. Do not make it automatic again.
